...
def bark():
  ...
  """
  name = (input("Enter dog's name: ")).capitalize()
  age = input("Enter dog's age: ")
  breed = (input("Enter dog's breed: ")).capitalize()
...
def bark_with_validation():
  ...
  if not(len(name)>=2 and name.isalpha()):
    print("Name error!")
  elif not(0<=int(age)<=15 and age.isnumeric()):
    print("Age error!")
  elif not(breed in ["Schnauzer", "Terrier", "Poodle", "Mastiff"]):
    print("Breed error!")
  else:
    print(name + ", the " + age + " year old " + breed + ', says, "Woof!"')
